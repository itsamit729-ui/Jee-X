...
export default function PublicProfile() {
  ...
  return <div className="pp-page">
    ...
    <main className="pp-main">
      ...
      {!username && <section className="pp-card pp-intro"><p className="pp-eyebrow">THE PEOPLE BEHIND THE RATINGS</p><h1>Every climb has a story.</h1><p className="pp-muted">Find a student by username to explore their rating, badges and contest journey. Only students who enable public profiles can be found.</p><Link to="/ranking">Explore the leaderboard →</Link></section>}
      {username && !profile && !error && <p role="status">Loading profile…</p>}
      {error && <section className="pp-card" role="alert"><h1>Profile unavailable</h1><p>{error}</p><p className="pp-muted">The profile may be private, unavailable, or the username may have changed.</p><button className="btn btn-secondary" onClick={() => setRetry(x => x+1)}>Try again</button></section>}
      {profile && <>
